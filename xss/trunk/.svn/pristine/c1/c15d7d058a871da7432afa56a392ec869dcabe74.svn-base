...
public abstract class MongodbBaseDaoImpl<T>  {
    ...
    public <T> void fetchCollections(final Query query, final CollectionIterator<T> iter) {

        mongoTemplate.execute(getEntityClass(), new CollectionCallback<T>() {
            @Override
            public T doInCollection(DBCollection dbCollection) throws MongoException, DataAccessException {
                ...
                while (cursor.hasNext()) {
                    ...
                    try {
                        ...
                        T t = (T)reader.read(getEntityClass(), v);
                        iter.onFetchOne(t);
                    } catch (Exception exp) {
                        exp.printStackTrace();
                    }
